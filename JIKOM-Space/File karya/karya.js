...
document.getElementById("menuBtn").addEventListener("click", () => {
    ...
    // DATA KARYA dipindahkan ke data.js

    function parseDateToParts(dateString) {
      // Mengharapkan format "DD Month YYYY", contoh: "28 August 2025"
    ...
    function renderSidebarKarya() {
      const recentSidebar = document.getElementById("recentSidebar");
      let recentHtml = "";
      daftarKarya.slice(0, 4).forEach(karya => {
        recentHtml += `
        <a href="../File detail/detail.html?type=karya&id=${karya.id}" class="flex gap-4 group items-start mb-2">
          <div class="w-[100px] h-[70px] rounded overflow-hidden shrink-0">
            <img src="${karya.gambar}" alt="" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" />
          </div>
          <div class="flex-1">
            <h4 class="text-[14px] font-semibold text-[#1e293b] group-hover:text-primary transition-colors line-clamp-2 leading-snug mb-1.5">${karya.judul}</h4>
            <div class="text-[12px] text-slate-400 font-medium">${karya.tanggal}</div>
          </div>
        </a>
        `;
      });
    ...
    function renderKarya(filter = "all", page = 1) {
      const grid = document.getElementById("karyaGrid");
      const itemsPerPage = 3;
      
      const filtered = daftarKarya.filter(karya => {
        if (filter === "all") return true;
        return karya.kategori.includes(filter);
      });
      ...
      let htmlContent = "";
      paginatedItems.forEach(karya => {
        const dataCategories = karya.kategori.join(",");
        const dateParts = parseDateToParts(karya.tanggal);

        htmlContent += `
        <article class="article-item block" data-categories="${dataCategories}">
          <div class="relative rounded-xl overflow-hidden mb-5 group">
            <a href="../File detail/detail.html?type=karya&id=${karya.id}" class="block w-full">
              <img src="${karya.gambar}" alt="${karya.judul}" class="w-full h-[320px] md:h-[400px] object-cover group-hover:scale-105 transition-transform duration-500" />
            </a>
          ...
          <a href="../File detail/detail.html?type=karya&id=${karya.id}" class="block">
            <h2 class="text-[22px] md:text-[24px] font-bold text-[#1e293b] mb-3 hover:text-primary transition-colors leading-snug">${karya.judul}</h2>
          </a>
          ...
          <a href="../File detail/detail.html?type=karya&id=${karya.id}" class="text-primary font-medium text-[14px] hover:text-orange-600 transition-colors inline-flex items-center gap-1.5">
            Read More &rarr;
          </a>
        </article>
        `;
      ...
      renderPaginationKarya(totalPages, page, filter);
    }

    function renderPaginationKarya(totalPages, currentPage, currentFilter) {
      ...
      if (!container) {
        ...
        document.getElementById("karyaGrid").parentElement.appendChild(container);
      }
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        html += `<button onclick="changePageKarya('${currentFilter}', ${i})" class="w-10 h-10 rounded-full border flex items-center justify-center text-[15px] font-medium transition-colors ${activeClass}">${i}</button>`;
      }
      
      if (currentPage < totalPages) {
        html += `<button onclick="changePageKarya('${currentFilter}', ${currentPage + 1})" class="w-10 h-10 rounded-full border border-slate-200 bg-white text-slate-600 hover:border-primary hover:text-primary flex items-center justify-center transition-colors">
    ...
    window.changePageKarya = function(filter, page) {
      const grid = document.getElementById("karyaGrid");
      const pagination = document.getElementById("paginationContainer");
      if(grid) grid.style.opacity = "0";
      if(pagination) pagination.style.opacity = "0";
      setTimeout(() => {
        renderKarya(filter, page);
        if(grid) grid.style.opacity = "1";
        if(pagination) pagination.style.opacity = "1";
