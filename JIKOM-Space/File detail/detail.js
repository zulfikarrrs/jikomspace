...
document.getElementById("menuBtn").addEventListener("click", () => {
    ...
    let item = null;
    let sourceList = [];
    let backHref = "../File karya/karya.html";

    if (type === "karya") {
      sourceList = daftarKarya;
      backHref = "../File karya/karya.html";
      item = daftarKarya.find(k => k.id === id);
    } else if (type === "publish") {
      sourceList = daftarPublish;
      backHref = "../File publish/publish.html";
    ...
    if (item) {
      ...
      related.forEach(r => {
        ...
        <a href="../File detail/detail.html?type=${type}&id=${r.id}" class="flex gap-4 group items-start">
      ...
      document.getElementById("heroSection").innerHTML = `
        <div class="flex items-center justify-center h-full">
          <div class="text-center text-white">
            ...
            <a href="../File index/index.html" class="bg-primary text-white px-6 py-3 rounded-lg font-medium hover:bg-orange-600 transition-colors">Kembali ke Beranda</a>
          </div>
        </div>`;
    }