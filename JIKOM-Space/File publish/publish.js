...
// ============================================
// ASPIRASI & WA BUTTON EXPAND/COLLAPSE
// ============================================
const initAspirasiWAButtons = () => {
...
// ============================================
// PUBLISH FILTER & PAGINATION
// ============================================
const initPublishFilters = () => {
...
// ============================================
// RENDER: Sidebar Publish
// ============================================
function renderSidebarPublish() {
  const recentSidebar = document.getElementById("recentSidebar");
  if (!recentSidebar || typeof daftarPublish === "undefined") return;

  let recentHtml = "";
  daftarPublish.slice(0, 4).forEach((publish) => {
    recentHtml += `
      <a href="detail.html?type=publish&id=${publish.id}" class="flex gap-4 group items-start mb-2">
...
function renderPublish(filter = "all", page = 1) {
  const grid = document.getElementById("publishGrid");
  if (!grid || typeof daftarPublish === "undefined") return;

  const itemsPerPage = 3;

  const filtered = daftarPublish.filter((publish) => {
    if (filter === "all") return true;
    return Array.isArray(publish.kategori)
      ? publish.kategori.includes(filter)
      : publish.kategori === filter;
  });
    ...
    htmlContent += `
      <article class="article-item block" data-kind="${Array.isArray(publish.kategori) ? publish.kategori.join(" ") : publish.kategori
        ...
        <div class="relative rounded-xl overflow-hidden mb-5 group">
          <a href="detail.html?type=publish&id=${publish.id}" class="block w-full">
        ...
        <a href="detail.html?type=publish&id=${publish.id}" class="block">
        ...
        <a href="detail.html?type=publish&id=${publish.id}" class="text-primary font-medium text-[14px] hover:text-orange-600 transition-colors inline-flex items-center gap-1.5">
          Read More &rarr;
        </a>
      </article>
    `;
...
function renderPaginationPublish(totalPages, currentPage, currentFilter) {
  ...
  if (!container) {
    ...
    container.className =
      "flex justify-start items-center gap-2 mt-8 mb-4 transition-opacity duration-200";
  ...
  if (currentPage < totalPages) {
    html += `<button onclick="changePagePublish('${currentFilter}', ${currentPage + 1
      })" class="w-10 h-10 rounded-full border border-slate-200 bg-white text-slate-600 hover:border-primary hover:text-primary flex items-center justify-center transition-colors">
