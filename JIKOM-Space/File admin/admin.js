// ===== AUTH =====
    ...
    function loadAll() {
      // Ambil data default dari data.js (jika ada), jika tidak pakai fallback
      const defaultK = typeof daftarKarya !== 'undefined' ? daftarKarya : DEFAULT_KARYA;
      const defaultP = typeof daftarPublish !== 'undefined' ? daftarPublish : DEFAULT_PUBLISH;
      
      const sk = localStorage.getItem('jikom_karya');
      const sp = localStorage.getItem('jikom_publish');
      
      let parsedK = sk ? JSON.parse(sk) : null;
      let parsedP = sp ? JSON.parse(sp) : null;
      
      // Sinkronisasi data: Pakai localStorage HANYA jika datanya lebih banyak (ada draft baru). 
      // Jika sama atau lebih sedikit, utamakan data.js agar update dari perangkat lain terbaca.
      if (parsedK && parsedK.length > defaultK.length) {
        karyaData = parsedK;
      } else {
        karyaData = JSON.parse(JSON.stringify(defaultK));
        localStorage.setItem('jikom_karya', JSON.stringify(karyaData));
      }

      if (parsedP && parsedP.length > defaultP.length) {
        publishData = parsedP;
      } else {
        publishData = JSON.parse(JSON.stringify(defaultP));
